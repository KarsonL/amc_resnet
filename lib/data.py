...
from lib.dataset import CIFAR10_IMG
...
def get_dataset(dset_name, batch_size, n_worker, data_root='./data'):
    ...
    if dset_name == 'cifar10':
        ...
        trainset = CIFAR10_IMG(root=data_root, train=True, transform=transform_train)
        
        train_loader = torch.utils.data.DataLoader(trainset, batch_size=batch_size, shuffle=True,
                                                   num_workers=n_worker)#, pin_memory=True, sampler=None)
        testset = CIFAR10_IMG(root=data_root, train=False, transform=transform_test)
...
def get_split_dataset(dset_name, batch_size, n_worker, val_size, data_root='../data/',
    ...
    if dset_name == 'cifar10':
        ...
        trainset = CIFAR10_IMG(root=data_root, train=True, transform=transform_train)#将CIFAR100改成 CIFAR10
        
        if use_real_val:  # split the actual val set
            valset = CIFAR10_IMG(root=data_root, train=False, transform=transform_test)
            n_val = len(valset)
            ...
            valset = CIFAR10_IMG(root=data_root, train=True, transform=transform_test)
