# -*- coding: utf-8 -*-
"""
Created on Tue Jan 19 20:23:27 2021
...
# -*- coding: utf-8 -*-
"""
Created on Tue Jan 19 19:32:35 2021
...
class Critic(nn.Module):
    def __init__(self, nb_states, nb_actions, hidden1=400, hidden2=300):
        ...
        #critic 2
        self.fc11_ = nn.Linear(nb_states, hidden1)
        self.fc12_ = nn.Linear(nb_actions, hidden1)
        self.fc2_ = nn.Linear(hidden1, hidden2)
        self.fc3_ = nn.Linear(hidden2, 1)
        self.relu_ = nn.ReLU()
    ...
    def forward(self, xs):
        ...
        #critic 2
        out_ = self.fc11_(x) + self.fc12_(a)
        out_ = self.relu_(out_)
        out_ = self.fc2_(out_)
        out_ = self.relu_(out_)
        out_ = self.fc3_(out_)
...
class Agent(object):
    def __init__(self, nb_states, nb_actions, args):
        ...
        # #critic 2
        # self.critic_2 = Critic(self.nb_states, self.nb_actions, **net_cfg)
        # self.critic_target_2 = Critic(self.nb_states, self.nb_actions, **net_cfg)
        # self.critic_optim_2 = Adam(self.critic_2.parameters(), lr=args.lr_c)
    ...
    def update_policy(self):
        ...
        with torch.no_grad():
            ...
            noise = (torch.randn_like(to_tensor(action_batch)) * self.policy_noise
                ).clamp(-self.noise_clip, self.noise_clip)
            
            next_action_batch = (self.actor_target(to_tensor(next_state_batch))+noise
                                 ).clamp(self.lbound, self.rbound)
            
            next_q_1, next_q_2= self.critic_target([
        ...
        target_q_batch = to_tensor(reward_batch) + \
                         self.discount * to_tensor(terminal_batch.astype(np.float)) * next_q

        # Critic update
        self.critic.zero_grad()       
        
        q_batch_1, q_batch_2 = self.critic([to_tensor(state_batch), to_tensor(action_batch)])
        
        value_loss = criterion(q_batch_1, target_q_batch) + criterion(q_batch_2, target_q_batch)
        value_loss.backward()        
        self.critic_optim.step()
        
        self.total_iter += 1
        if self.total_iter%self.actor_freq == 0:
