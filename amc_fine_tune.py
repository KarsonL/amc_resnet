...
def parse_args():
    ...
    parser.add_argument('--model', default='resnet56', type=str, help='name of the model to train')
    ...
    parser.add_argument('--data_root', default='H:/share/cifar10/', type=str, help='dataset path')#'H:/share/imagenet/'
    # resume
    parser.add_argument('--ckpt_path', default=None, type=str, help='checkpoint path to resume from')
    # run eval
    parser.add_argument('--eval', action='store_true', help='Simply run eval',default=0)
...
def get_model(n_class):
    ...
    if args.model == 'mobilenet':
        ...
        net = ResNet56(num_classes=n_class)
    elif args.model == 'mobilenet_0.5flops':
        from models_.mobilenet import MobileNet
        net = MobileNet(n_class=n_class, profile='0.5flops')
    else:
...
def test(epoch, test_loader, args, save=True):
    global best_acc
    net.eval()
    ...
    if save:
        ...
        }, is_best, args, checkpoint_dir=log_dir)
...
def save_checkpoint(state, is_best,args,checkpoint_dir='.'):
    filename = os.path.join(checkpoint_dir, 'ckpt.pth.tar')
    print('=> Saving checkpoint to {}'.format(filename))
    torch.save(state, filename)
    if is_best:
        shutil.copyfile(filename, filename.replace('.pth.tar', '.best.pth.tar'))
        ckpt_name = args.ckpt_path.replace('.pth.tar','_ft.pth.tar')
        shutil.copyfile(filename, ckpt_name)
...
if __name__ == '__main__':
    ...
    del net
    net = get_model(n_class)  # real training
    ...
    if args.eval:  # just run eval
        ...
        test(0, val_loader, args, save=False)
    else:  # train
        ...
        for epoch in range(start_epoch, start_epoch + args.n_epoch):
            ...
            if epoch%100 == 0:
                epo = start_epoch
            # else:           
            lr = adjust_learning_rate(optimizer, epo)
            epo += 1
            
            train(epoch, train_loader)
            test(epoch, val_loader, args)
