...
from env.rewards import *         #import acc_reward, acc_flops_reward
import math
...
class ChannelPruningEnv:
    ...
    """
    def __init__(self, model, checkpoint, data, preserve_ratio, args, n_data_worker=0,
                 batch_size=128, export_model=False, use_new_input=False):
        # default setting
        ...
        # self.m_acc = self._validate(self.val_loader, self.model)
        self.m_acc = 80.599
        ...
        self.reward = eval(args.reward) #这里的表达式是 acc_reward(net, acc, flops)
    ...
    def train_model(self,loss_func,optimizer,scheduler):
        ...
        m_acc = self._validate(self.val_loader, self.model)
        self.m_acc = int(m_acc*10)
    ...
    def step(self, action):
        ...
        if self._is_final_layer():
            ...
            current_flops = self._cur_flops()  # current all layers flops, not the current layer flops
            acc_t1 = time.time()
            acc = self._validate(self.val_loader, self.model) # validate after all layer pruned
            acc_t2 = time.time()
            self.val_time = acc_t2 - acc_t1
            compress_ratio = current_flops * 1. / self.org_flops  #the whole compress ratio of the model
            info_set = {'compress_ratio': compress_ratio, 'accuracy': acc, 'strategy': self.strategy.copy()}
            reward = self.reward(self, acc, current_flops) #env.reward.acc_reward(net, acc, flops)
            # reward = self.reward(self, (acc/self.m_acc)*100, current_flops) #env.reward.acc_reward(net, acc, flops)
        ...
        # self.layer_embedding[self.cur_ind][-2] = self._cur_reduced() * 1. / self.org_flops  # reduced
        # self.layer_embedding[self.cur_ind][-1] = sum(self.flops_list[self.cur_ind + 1:]) * 1. / self.org_flops  # rest
    ...
    def reset(self):
        ...
        self.layer_embedding[:, -1] = 1. # a_t-1  $
        self.layer_embedding[:, -2] = 0. # rest     $
        self.layer_embedding[:, -3] = 0. # reduce   $
        
        self.layer_embedding[:, -4] = self.m_acc #新增
        
        # # self.layer_embedding[:, -1] = 1. # a_t-1
        # self.layer_embedding[:, -1] = 0. # rest
        # self.layer_embedding[:, -2] = 0. # reduce
        
        obs = self.layer_embedding[0].copy()
        obs[-2] = sum(self.wsize_list[1:]) * 1. / sum(self.wsize_list) # rest is all $
        # obs[-1] = sum(self.wsize_list[1:]) * 1. / sum(self.wsize_list) # rest is all
        self.extract_time = 0
        self.fit_time = 0
        self.val_time = 0
        # for share index
    ...
    def prune_kernel(self, op_idx, preserve_ratio, preserve_idx=None):
        ...
        op = m_list[op_idx]
        assert (preserve_ratio <= 1. and preserve_ratio > 0.)
        ...
        n, c = op.weight.size(0), op.weight.size(1) #C_out, C_in
        ...
        # reconstruct, X, Y <= [N, C] 特征重建
        # masked_X = X[:, mask]
        if weight.shape[2] == 1:  # 1x1 conv or fc
            from lib.utils import least_square_sklearn #最小二乘法
            masked_X = X[:, mask]
        ...
        elif weight.shape[2] == 3:  # 3x3 conv
            from lib.utils import least_square_sklearn #最小二乘法
            # masked_X = X[:, mask]
            X = X.reshape(X.shape[0], weight.shape[1], -1)
            X = X[:, mask]
            masked_X = X.reshape(X.shape[0], -1)          
            
            rec_weight = least_square_sklearn(X=masked_X, Y=Y)
            rec_weight = rec_weight.reshape(-1, 3, 3, d_prime)  # (C_out, K_h, K_w, C_in')
            rec_weight = np.transpose(rec_weight, (0, 3, 1, 2))  # (C_out, C_in', K_h, K_w)
        else:
    ...
    def _build_index(self):
        ...
        for i, m in enumerate(self.model.modules()):
            if type(m) in self.prunable_layer_types:
                if type(m) == nn.Conv2d and m.groups == m.in_channels:  # depth-wise conv, buffer #深度可分离不剪枝
                    this_buffer_list.append(i)
                else:  # really prunable
                    self.prunable_idx.append(i) #保存可剪枝的
                    self.prunable_ops.append(m) #保存可剪枝的操作层
                    self.layer_type_dict[i] = type(m)
                    self.buffer_dict[i] = this_buffer_list #保存不可剪枝的conv
        ...
        if self.args.model == 'mobilenetv2':  # TODO: to be tested! Share index for residual connection
            ...
            print('=> Conv layers to share channels: {}'.format(self.shared_idx)) #同通道数的要共享
        elif 'resnet'in self.args.model and 'cifar' in self.args.dataset:  # TODO: to be tested! Share index for residual connection
            # connected_idx = [2, 4, 6, 8, 10, 12, 14, 16, 18]  # to be partitioned #在可剪枝层的相对位置
            connected_idx = []
            n_layers = int(self.args.model[-2:])-2
            for i in range(int(n_layers/2)):
                connected_idx.append(i*2 + 2)
            print('connected_idx : ', connected_idx)
        ...
        with torch.no_grad():
            for i_b, (input, target) in enumerate(self.train_loader):  # use image from train set
                ...
                if i_b == 0:  # first batch
                    for idx in self.prunable_idx + self.buffer_idx: #所有层
                ...
                def im2col(x, conv):
                    x_unfold = torch.nn.functional.unfold(x, kernel_size=conv.kernel_size, stride=conv.stride, padding=conv.padding)
                    return x_unfold
                
                for idx in self.prunable_idx: # 保存 可剪枝层的 输入输出特征
                    ...
                    if len(f_in_np.shape) == 4:  # conv
                        ...
                        # elif m_list[idx].weight.size(3) > 1:  # normal conv 3x3
                            # f_in2save, f_out2save = f_in_np, f_out_np
                        else:  # 1x1 conv
                            # assert f_out_np.shape[2] == f_in_np.shape[2]  # now support k=3
                            randx = np.random.randint(0, f_out_np.shape[2] - 0, self.n_points_per_layer) #???为什么要随机
                            ...
                            # 改， 支持3x3
                            if m_list[idx].weight.size(3) > 1:
                                # w, h = f_in_np.size(2), f_in_np.size(3)
                                
                                f_in_np = im2col(m_list[idx].input_feat.data.cpu().clone(), m_list[idx]).squeeze()
                        ...
                        assert len(f_in_np.shape) == 2 #linear
    ...
    '''
    状态的设计： st=[t,l_type,c,n,stride,k,w,h,reduce,rest,a_t-1] 没有了FLOPs[t]，多了layer_type
    '''
    ...
    def _build_state_embedding(self):
        ...
        for i, ind in enumerate(self.prunable_idx):
            ...
            if type(m) == nn.Conv2d:
                ...
                this_state.append(np.prod(m.weight.size()))  # weight size W*H
